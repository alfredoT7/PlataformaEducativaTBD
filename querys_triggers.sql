...
CREATE OR REPLACE FUNCTION trigger_registrar_log(id_event_p INTEGER)
RETURNS TRIGGER AS $$
BEGIN
INSERT INTO log_bd(fecha_event,hora_event,dato_viejo,dato_nuevo,tabla,pid_usuario,id_event)
VALUES (SELECT CURRENT_DATE,SELECT LOCALTIME,OLD,NEW,TG_TABLE_NAME,select pg_backend_pid(),id_event_p);
...
ALTER TABLE log_BD DROP COLUMN usuarion;
...
v_id_event INTEGER;
BEGIN
SELECT id_event INTO v_id_event
FROM evento
WHERE tipo_evento = TG_OP;

INSERT INTO log_bd(fecha_event, hora_event, dato_viejo, dato_nuevo, tabla, pid_usuario, id_event)
VALUES (CURRENT_DATE, LOCALTIME, OLD, NEW, TG_TABLE_NAME, pg_backend_pid(), v_id_event);
